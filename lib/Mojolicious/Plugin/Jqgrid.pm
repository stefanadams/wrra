...
sub jqgrid_renderer {
  ...
  my $rs = $c->stash->{result};
  $$output = {
    page => $rs->pager->current_page||1,
    total => $rs->pager->last_page||1,
    records => $rs->pager->total_entries||0,
#    rows => [Mojo::JSON->new->encode($rs->all)],
  }
}
...
sub register {
  ...
  $app->types->type(jqgrid => 'x-application/jqgrid');
  $app->renderer->add_handler(jqgrid => \&jqgrid_renderer);
  $app->helper(
    render_jqgrid => sub {
      shift->render(handler => 'jqgrid', @_);
    }
  );
}
