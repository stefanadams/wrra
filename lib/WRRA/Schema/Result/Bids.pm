...
sub _colmodel { qw/id bid_r bid bidtime bidder.id bidder.nameid bidder.phone item.id item.year item.number item.nameid item.value item.bellringer bidage/ }
...
#sub _create { $_[1] }
sub _search { $_[1]->year('item') }
#sub _update { $_[1] }
#sub _delete { $_[1] }

# The relationships associated with this result (table)
our $relationships = [qw/item bidder/];
